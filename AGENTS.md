...
Guía para agentes y personas que modifiquen MediaFetch. Lo que hace la app desde el lado del usuario está en el [README](README.md).

## Qué es

Una app para Windows 10 y 11 que baja música y videos de YouTube con yt-dlp y FFmpeg, saca el audio de videos de la compu y los transcribe con Whisper (vía Groq). Se distribuye como un ZIP portátil en los Releases: `MediaFetch.exe` más `app/` y `bin/`, con Node, yt-dlp y FFmpeg adentro. Quien la usa no instala ni baja nada: descomprime y hace doble clic en `MediaFetch.exe`, sin consola.
...
| Archivo | Qué es |
| --- | --- |
| `launcher/MediaFetch.cs` | `MediaFetch.exe`: lanzador de ventana (sin consola) en C# 5 para .NET Framework 4, que ya viene con Windows. Corre `bin\node.exe app\server.js` oculto y, si sale con error, muestra su stderr en un cartel. `launcher/mediafetch.ico` es el logo de `index.html`. |
...
| `tools/build.js` | Baja a `bin/` lo que falte y compila `MediaFetch.exe`; con `--zip` arma `dist/MediaFetch-win64.zip`. |
...
No se versionan, y no pueden terminar en el ZIP: `bin/*` salvo `download.bat`, `MediaFetch.exe`, `dist/`, `app/settings.json` (tiene la clave de Groq) y `app/server.log`.

## Cómo se corre

- `node tools/build.js`: prepara el repo como la carpeta portátil (Node 22+, en Windows o WSL). Después, `MediaFetch.exe`.
- `node app/server.js`: el lanzador que usa `MediaFetch.exe`. Con `--no-window` no abre la ventana; con `--serve` corre el servidor en primer plano, para depurar, en http://127.0.0.1:17865.
- Variables: `MEDIAFETCH_PORT` cambia el puerto (para probar sin chocar con una instancia abierta) y `MEDIAFETCH_GROQ_API` la URL de Groq (para probar la transcripción contra un servidor falso).

## Arquitectura

- **Arranque.** `MediaFetch.exe` corre el lanzador de Node, que hace ping a `/api/ping`. Si responde un servidor con otro `build` (el mtime de `server.js`) y está libre, le pide `/api/quit` y arranca uno nuevo: editar `server.js` alcanza para que la próxima apertura use el código nuevo. El servidor corre desacoplado con `--serve`, con su salida en `server.log`, y la ventana es Edge, Chrome o Brave en modo `--app`.
...
- **Nubes.** Al arrancar, `findClouds()` busca las carpetas que sincronizan OneDrive (sus variables de entorno), Google Drive para escritorio (`G:\Mi unidad` o su carpeta espejo, solo si está instalado), Dropbox (su `info.json`) e iCloud. Si encuentra alguna, la ventana muestra "Guardar en: Música · OneDrive…" (`places` en las opciones) para elegir con un clic. La carpeta de descargas nunca cambia sola. Cuando la carpeta está en una nube, yt-dlp baja a `%TEMP%\MediaFetch` (`-P temp:`) y mueve solo lo terminado, así el programa de la nube no sube archivos a medio bajar. No se suben archivos por API: lo sube el programa de cada nube.
...
- **Archivos del usuario.** Nunca se pisa nada: `freeName()` busca un nombre libre. Las descargas van por defecto a `Música\MediaFetch`, fuera de la carpeta de MediaFetch (se reemplaza entera al actualizar), y esa carpeta por defecto no se guarda en `settings.json`, así sigue siendo la de cada usuario si MediaFetch se copia a otra PC. Para buscar carpetas, solo rutas conocidas: nunca recorras las letras de unidad, porque una unidad de red desconectada puede trabar el servidor un minuto.
...
**Firma.** `MediaFetch.exe` no está firmado, así que Windows avisa la primera vez ("Windows protegió tu PC"). Si se agrega una firma, va en el workflow entre la compilación y el ZIP, porque `tools/build.js` mete `MediaFetch.exe` en el ZIP. Aun firmado, SmartScreen avisa hasta que el editor junta reputación.
...
2. `node --test`.
3. `git diff --check`.
4. Lo que depende de Windows (los `.exe`, `taskkill`, `explorer`, PowerShell, el lanzador) se prueba en Windows. Desde WSL, cmd.exe no acepta `\\wsl.localhost\...` como carpeta actual: extraé `dist/MediaFetch-win64.zip` en una carpeta de Windows fuera de `%TEMP%` y corré desde ahí con `cmd.exe /c`. Para no abrir ventanas, usá `--serve` con `MEDIAFETCH_PORT` y una conexión a `/api/events` que haga de ventana.
5. Videos de prueba: `https://www.youtube.com/watch?v=jNQXAC9IVRw` ("Me at the zoo", 19 s; el viejo de prueba de yt-dlp, `BaW_jenozKc`, ya no existe) y, para cancelar a mitad de una descarga, `https://www.youtube.com/watch?v=aqz-KE-bpKQ` (Big Buck Bunny, largo y de licencia libre). Mandá las descargas de prueba a una carpeta propia con `options.outputDir`, no a la Música del usuario. Para probar las nubes, no uses la del usuario (lo que se guarda ahí se sube a su cuenta): arrancá el servidor con `OneDrive` y `OneDriveConsumer` apuntando a una carpeta de prueba.
